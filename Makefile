...
release: build-release
	@if ! git rev-parse --is-inside-work-tree >/dev/null 2>&1; then \
		echo "git repository required for release"; exit 1; \
	fi; \
	@if ! git diff --quiet; then \
		echo "clean working tree required for release"; exit 1; \
	fi; \
	@tag=v$(VERSION); \
	@echo "Tagging $$tag"; \
	...
	@if ! command -v gh >/dev/null 2>&1; then \
		echo "gh CLI not found; create release manually with artifacts in $(RELEASE_DIR)"; exit 1; \
	fi; \
